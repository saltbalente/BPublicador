...
function setupEventListeners() {
    ...
    // Image Forms
    safeAddEventListener('imageForm', 'submit', handleGenerateImage);
    safeAddEventListener('imageConfigForm', 'submit', handleImageConfigForm);
    safeAddEventListener('manualImageForm', 'submit', handleManualImageForm);
    
    // Image Configuration
    safeAddEventListener('keywordSelect', 'change', handleKeywordSelect);
    
    // Setup image provider listener
    setupImageProviderListener();
...
let imageConfig = {
    ...
    geminiPersonGeneration: true
};

// Keyword-specific image configurations
let keywordImageConfigs = {};

async function handleImageConfigForm(e) {
    e.preventDefault();
    
    // Update global configuration
    imageConfig = {
        imagesPerContent: parseInt(document.getElementById('imagesPerContent').value),
        imageSize: document.getElementById('imageSize').value,
        imageQuality: document.getElementById('imageQuality').value,
        imagePlacement: document.getElementById('imagePlacement').value,
        imageStyle: document.getElementById('imageStyle').value,
        autoGenerateImages: document.getElementById('autoGenerateImages').checked,
        includeFeaturedImage: document.getElementById('includeFeaturedImage').checked,
        imageProvider: document.getElementById('imageProvider').value,
        geminiAspectRatio: document.getElementById('geminiAspectRatio').value,
        geminiSafetyLevel: document.getElementById('geminiSafetyLevel').value,
        geminiPersonGeneration: document.getElementById('geminiPersonGeneration').checked
    };
    
    try {
        const response = await apiRequest('/image-generation/config', {
            method: 'POST',
            body: JSON.stringify(imageConfig)
        });
        
        if (response) {
            showAlert('Configuración de imágenes guardada exitosamente', 'success');
...
async function handleKeywordSelect() {
    const keywordId = document.getElementById('keywordSelect').value;
    const configDiv = document.getElementById('keywordImageConfig');
    
    if (keywordId) {
        configDiv.style.display = 'block';
        
        // Load existing configuration for this keyword
        const existingConfig = keywordImageConfigs[keywordId];
        if (existingConfig) {
            document.getElementById('keywordImagePrompt').value = existingConfig.prompt || '';
            document.getElementById('keywordImageStyle').value = existingConfig.style || '';
            document.getElementById('keywordImageCount').value = existingConfig.count || '';
        } else {
            // Clear form for new keyword
            document.getElementById('keywordImagePrompt').value = '';
            document.getElementById('keywordImageStyle').value = '';
            document.getElementById('keywordImageCount').value = '';
        }
    } else {
...
async function saveKeywordImageConfig() {
    const keywordId = document.getElementById('keywordSelect').value;
    if (!keywordId) {
        showAlert('Selecciona una keyword primero', 'warning');
        return;
    }
    
    const config = {
        keyword_id: parseInt(keywordId),
        prompt: document.getElementById('keywordImagePrompt').value,
        style: document.getElementById('keywordImageStyle').value,
        count: document.getElementById('keywordImageCount').value
    };
    
    try {
        const response = await apiRequest('/image-generation/keyword-config', {
            method: 'POST',
            body: JSON.stringify(config)
        });
        
        if (response) {
            keywordImageConfigs[keywordId] = config;
            showAlert('Configuración guardada para la keyword', 'success');
            updateImageStats();
        }
    } catch (error) {
        showAlert('Error al guardar configuración de keyword', 'danger');
        console.error('Error saving keyword image config:', error);
    }
}

async function handleManualImageForm(e) {
    e.preventDefault();
    
    const keywordId = document.getElementById('manualKeywordSelect').value;
    const count = parseInt(document.getElementById('manualImageCount').value);
    const prompt = document.getElementById('manualImagePrompt').value;
    
    if (!keywordId) {
        showAlert('Selecciona una keyword base', 'warning');
        return;
    }
    ...
    try {
        const response = await apiRequest('/image-generation/generate-manual', {
            method: 'POST',
            body: JSON.stringify({
                keyword_id: parseInt(keywordId),
                prompt: prompt,
        ...
        if (response.success) {
            showAlert(`${response.images_generated} imagen(es) generada(s) exitosamente`, 'success');
            document.getElementById('manualImageForm').reset();
            await loadImageGallery();
            updateImageStats();
        } else {
            showAlert(`Error: ${response.detail || 'Respuesta inesperada del servidor'}`, 'danger');
        }
    } catch (error) {
        const errorMessage = error.detail || error.message || 'Error desconocido';
        showAlert(`Error al generar imágenes: ${errorMessage}`, 'danger');
        console.error('Error generating manual images:', error);
    } finally {
        showLoading(false);
    }
}
...
async function loadImageConfiguration() {
    try {
        ...
        const globalConfig = await apiRequest('/image-generation/config');
        if (globalConfig) {
            imageConfig = globalConfig;
            populateImageConfigForm();
        } else {
            // Load from localStorage if no server config
            const savedConfig = localStorage.getItem('imageConfig');
            if (savedConfig) {
                imageConfig = JSON.parse(savedConfig);
                populateImageConfigForm();
            }
        }
        
        // Load keyword configurations
        const keywordConfigs = await apiRequest('/image-generation/keyword-configs');
        if (keywordConfigs) {
            keywordImageConfigs = keywordConfigs;
        }
        
        // Load keywords for selects
        await loadKeywordsForImageConfig();
...
function populateImageConfigForm() {
    document.getElementById('imagesPerContent').value = imageConfig.imagesPerContent;
    document.getElementById('imageSize').value = imageConfig.imageSize;
    document.getElementById('imageQuality').value = imageConfig.imageQuality;
    document.getElementById('imagePlacement').value = imageConfig.imagePlacement;
    document.getElementById('imageStyle').value = imageConfig.imageStyle;
    document.getElementById('autoGenerateImages').checked = imageConfig.autoGenerateImages;
    document.getElementById('includeFeaturedImage').checked = imageConfig.includeFeaturedImage;
    
    // Configuraciones de Gemini
    if (document.getElementById('imageProvider')) {
        document.getElementById('imageProvider').value = imageConfig.imageProvider || 'gemini';
    }
    if (document.getElementById('geminiAspectRatio')) {
        document.getElementById('geminiAspectRatio').value = imageConfig.geminiAspectRatio || '1:1';
    }
    if (document.getElementById('geminiSafetyLevel')) {
        document.getElementById('geminiSafetyLevel').value = imageConfig.geminiSafetyLevel || 'BLOCK_MEDIUM_AND_ABOVE';
    }
    if (document.getElementById('geminiPersonGeneration')) {
        document.getElementById('geminiPersonGeneration').checked = imageConfig.geminiPersonGeneration !== false;
    }
    
    // Mostrar/ocultar configuraciones específicas de Gemini
...
function setupImageProviderListener() {
    const providerSelect = document.getElementById('imageProvider');
    if (providerSelect) {
        providerSelect.addEventListener('change', toggleGeminiSettings);
    }
}

async function loadKeywordsForImageConfig() {
    try {
        console.log('loadKeywordsForImageConfig: Starting...');
        const keywords = await apiRequest('/keywords/');
        console.log('loadKeywordsForImageConfig: Keywords received:', keywords);
        
        if (keywords) {
            const keywordSelect = document.getElementById('keywordSelect');
            const manualKeywordSelect = document.getElementById('manualKeywordSelect');
            
            console.log('loadKeywordsForImageConfig: Elements found:', {
                keywordSelect: !!keywordSelect,
                manualKeywordSelect: !!manualKeywordSelect
            });
            
            if (!keywordSelect || !manualKeywordSelect) {
                console.error('loadKeywordsForImageConfig: Select elements not found!');
                return;
            }
            
            // Clear existing options
            keywordSelect.innerHTML = '<option value="">Selecciona una keyword...</option>';
            manualKeywordSelect.innerHTML = '<option value="">Selecciona una keyword...</option>';
            
            console.log('loadKeywordsForImageConfig: Populating dropdowns with', keywords.length, 'keywords');
            
            keywords.forEach((keyword, index) => {
                console.log(`loadKeywordsForImageConfig: Adding keyword ${index + 1}:`, keyword);
                
                const option1 = document.createElement('option');
                option1.value = keyword.id;
                option1.textContent = keyword.keyword;
                keywordSelect.appendChild(option1);
                
                const option2 = document.createElement('option');
                option2.value = keyword.id;
                option2.textContent = keyword.keyword;
                manualKeywordSelect.appendChild(option2);
            });
            
            console.log('loadKeywordsForImageConfig: Dropdowns populated successfully');
            console.log('loadKeywordsForImageConfig: Final option counts:', {
                keywordSelect: keywordSelect.options.length,
                manualKeywordSelect: manualKeywordSelect.options.length
            });
        } else {
            console.warn('loadKeywordsForImageConfig: No keywords received');
        }
    } catch (error) {
        console.error('Error loading keywords for image config:', error);
    }
}

async function updateImageStats() {
    try {
        const stats = await apiRequest('/image-generation/stats');
        if (stats) {
            document.getElementById('totalImages').textContent = stats.total_images || 0;
            document.getElementById('monthlyImages').textContent = stats.monthly_images || 0;
            document.getElementById('configuredKeywords').textContent = Object.keys(keywordImageConfigs).length;
        }
    } catch (error) {
...
async function handleGenerateImage(e) {
    ...
    // Try to get prompt from quick form first, then fallback to old form
    const quickPrompt = document.getElementById('quickImagePrompt');
    const oldPrompt = document.getElementById('imagePrompt');
    const styleSelect = document.getElementById('quickImageStyle');
    
    const prompt = quickPrompt ? quickPrompt.value : (oldPrompt ? oldPrompt.value : '');
    const style = styleSelect ? styleSelect.value : 'realistic';
    
    if (!prompt) {
        showAlert('Por favor ingresa una descripción para la imagen', 'warning');
        return;
    }
    ...
    try {
        const response = await apiRequest('/image-generation/generate', {
            method: 'POST',
            body: JSON.stringify({
                prompt: prompt,
                style: style
        ...
        const errorMessage = error.detail || error.message || 'Error desconocido';
        showAlert(`Error al generar imagen: ${errorMessage}`, 'danger');
...
async function loadImageGallery() {
    try {
        const images = await apiRequest('/image-generation/images');
        if (images) {
...
function displayImageGallery(images) {
    ...
    if (!images || images.length === 0) {
        gallery.innerHTML = '<div class="col-12 text-center"><p>No hay imágenes disponibles</p></div>';
        return;
    }
    ...
    images.forEach(image => {
        ...
        col.innerHTML = `
            <div class="image-item">
                <img src="${image.url}" alt="${image.prompt}" class="img-fluid">
