...
// API Endpoint Mapping for migrated backend
const API_ENDPOINTS = {
    posts: '/posts/',
    content: '/posts/', // Map content to posts
    keywords: '/posts/tags/popular', // Map keywords to popular tags
    themes: '/admin/themes',
    ...
    generate: {
        ...
        seo: '/generate/seo-analysis',
        history: '/generate/history'
    }
};

// Mock data for missing endpoints during migration
function getMockData(endpoint) {
    if (endpoint.includes('visual-config')) {
        return {
            success: true,
            config: {
                theme: 'light',
    ...
    if (endpoint.includes('keywords')) {
        return {
            success: true,
            keywords: [
                { id: 1, keyword: 'ejemplo', priority: 'medium', search_volume: 1000, difficulty: 'easy' },
                { id: 2, keyword: 'demo', priority: 'high', search_volume: 2000, difficulty: 'medium' }
            ]
        };
    }
...
async function apiRequest(endpoint, options = {}) {
    // Map legacy endpoints to new backend endpoints
    let mappedEndpoint = endpoint;
    
    // Handle endpoint mapping
    if (endpoint.startsWith('/content')) {
        mappedEndpoint = endpoint.replace('/content', API_ENDPOINTS.content);
    } else if (endpoint.startsWith('/keywords')) {
        mappedEndpoint = API_ENDPOINTS.keywords;
    } else if (endpoint.startsWith('/themes')) {
        mappedEndpoint = endpoint.replace('/themes', API_ENDPOINTS.themes);
    } else if (endpoint.startsWith('/settings')) {
        mappedEndpoint = endpoint.replace('/settings', API_ENDPOINTS.settings);
    } else if (endpoint.startsWith('/stats') || endpoint.startsWith('/analytics')) {
        mappedEndpoint = API_ENDPOINTS.stats;
    } else if (endpoint.startsWith('/visual-config')) {
        // Return mock data for visual config
    ...
    console.log('apiRequest - original endpoint:', endpoint);
    console.log('apiRequest - mapped endpoint:', mappedEndpoint);
    console.log('apiRequest - final URL:', url);
    console.log('apiRequest - token from localStorage:', token ? 'EXISTS' : 'NULL');
    
    // For some endpoints, we don't require authentication during development
    const publicEndpoints = ['/health', '/admin/health'];
    const isPublicEndpoint = publicEndpoints.some(pe => mappedEndpoint.includes(pe));
    ...
    try {
        ...
        if (!response.ok) {
            // For 404 errors on missing endpoints, return mock data
            if (response.status === 404 && (endpoint.includes('visual-config') || endpoint.includes('keywords') || endpoint.includes('analytics') || endpoint.includes('dashboard'))) {
                console.log('Endpoint not found, returning mock data for:', endpoint);
                return getMockData(endpoint);
            }
        ...
        // Return mock data for development when endpoints are missing
        if (endpoint.includes('visual-config') || endpoint.includes('keywords') || endpoint.includes('analytics') || endpoint.includes('dashboard')) {
            console.log('API error, returning mock data for:', endpoint);
            return getMockData(endpoint);
        }
